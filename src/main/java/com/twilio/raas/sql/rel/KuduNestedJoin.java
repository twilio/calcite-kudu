...
/**
 * Implements a join algorithm between two {@link KuduToEnumerableRel} objects using
...
public class KuduNestedJoin extends Join implements EnumerableRel {
    ...
    @Override
    public KuduNestedJoin copy(final RelTraitSet traitSet,
    ...
    public RelOptCost computeSelfCost(
        ...
        // If row on the left doesn't have an estimate, or if right's estimate is > left,
        // return infinite. Our join algorithm will not perform well.
        if (Double.isInfinite(leftRowCount)
            || Double.isInfinite(right.estimateRowCount(mq))
