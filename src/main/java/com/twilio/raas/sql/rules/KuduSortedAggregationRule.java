...
public abstract class KuduSortedAggregationRule extends KuduSortRule {
  ...
  private static final RelOptRuleOperand LIMIT_OPERAND =
      operand(EnumerableLimit.class,
        some(operand(Sort.class,
          some(operand(Aggregate.class,
              some(operand(KuduToEnumerableRel.class,
                  some(operand(Project.class,
                      some(operand(Filter.class,
  ...
  public static final RelOptRule SORTED_AGGREGATION_LIMIT_RULE =
      new KuduSortedAggregationWithLimitRule(RelFactories.LOGICAL_BUILDER);

  public static final RelOptRule SORTED_AGGREGATION_RULE =
      new KuduSortedAggregationWithoutLimitRule(RelFactories.LOGICAL_BUILDER);

  public KuduSortedAggregationRule(RelOptRuleOperand operand, RelBuilderFactory relBuilderFactory
      , String description) {
    super(operand, relBuilderFactory, description);
  }

  protected void perform(final RelOptRuleCall call,  final Optional<EnumerableLimit> originalLimit,
                      final Sort originalSort, final Aggregate originalAggregate,
                      final KuduToEnumerableRel kuduToEnumerableRel, final Project project,
  ...
  /**
   * Rule to match an aggregation over a sort over a prefix of the primary key columns
   * with a limit
   */
  public static class KuduSortedAggregationWithLimitRule extends KuduSortedAggregationRule {
    ...
    public void onMatch(final RelOptRuleCall call) {
      ...
      perform(call, Optional.of(originalLimit), originalSort, originalAggregate,
          kuduToEnumerableRel, project, filter, query);
    }
  }
  ...
  public static class KuduSortedAggregationWithoutLimitRule extends KuduSortedAggregationRule {
    ...
    public void onMatch(final RelOptRuleCall call) {
      ...
      final KuduToEnumerableRel kuduToEnumerableRel =
          (KuduToEnumerableRel) call.getRelList().get(2);
      ...
      perform(call, Optional.empty(), originalSort, originalAggregate,
          kuduToEnumerableRel, project, filter, query);
    }
  }
