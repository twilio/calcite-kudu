...
import com.twilio.raas.sql.SqlUtil;
...
public class SqlColumnDefNode extends AbstractSqlNode {
  ...
  public SqlColumnDefNode(
    ...
    if (compression != null) {
      ...
      columnSchemaBuilder.compressionAlgorithm(
        ColumnSchema.CompressionAlgorithm.valueOf(compressionString));
    }
    if (blockSize != -1) {
  ...
  public SqlNode clone(SqlParserPos pos) {
    return new SqlColumnDefNode(pos, columnName, dataType, isNullable, isPk, isRowTimestamp,
      sortOrder, defaultValueExp, encoding, compression, blockSize, comment);
  }
  ...
  public boolean equalsDeep(SqlNode node, Litmus litmus) {
    ...
    if (!this.columnName.equalsDeep(that.columnName, litmus)
      || !this.dataType.equalsDeep(that.dataType, litmus)
      || this.isNullable != that.isNullable
      || this.isPk != that.isPk
      || this.sortOrder != that.sortOrder
      || this.isRowTimestamp != that.isRowTimestamp
      || (this.defaultValueExp!=null
        && !this.defaultValueExp.equalsDeep(that.defaultValueExp, litmus))
