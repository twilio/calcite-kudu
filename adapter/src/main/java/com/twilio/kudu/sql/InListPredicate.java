...
public final class InListPredicate extends CalciteKuduPredicate {
  ...
  public KuduPredicate toPredicate(final ColumnSchema columnSchema, final boolean invertValue) {
    ...
    case STRING:
      final List<String> stringValues = this.values.stream().map(o -> o.toString()).collect(Collectors.toList());
      return KuduPredicate.newInListPredicate(columnSchema, stringValues);
    case BOOL:
      final List<Boolean> booleanValues = this.values.stream().map(o -> (Boolean) o).collect(Collectors.toList());
      return KuduPredicate.newInListPredicate(columnSchema, booleanValues);
    case INT8:
      final List<Number> byteValues = this.values.stream().map(o -> {
        if (invertValue) {
    ...
    case INT16:
      final List<Number> shortValues = this.values.stream().map(o -> {
        if (invertValue) {
    ...
    case INT32:
      final List<Number> intValues = this.values.stream().map(o -> {
        if (invertValue) {
    ...
    case UNIXTIME_MICROS:
      ...
      if (this.values.get(0) instanceof Timestamp) {
        timestampsInMicros = this.values.stream().map(original -> {
          final long ts = ((Timestamp) original).toInstant().toEpochMilli();
        ...
        throw new IllegalArgumentException(
            String.format("Cannot convert a %s into a UNIXTIME_MICROS for Kudu", this.values.get(0).getClass()));
      }

      return KuduPredicate.newInListPredicate(columnSchema, timestampsInMicros);

    case INT64:
      final List<Long> longValues = this.values.stream().map(o -> {
    ...
    case FLOAT:
      final List<Float> floatValues = this.values.stream().map(o -> ((Number) o).floatValue())
          .collect(Collectors.toList());

      return KuduPredicate.newInListPredicate(columnSchema, floatValues);

    case DOUBLE:
      final List<Double> doubleValues = this.values.stream().map(o -> ((Number) o).doubleValue())
          .collect(Collectors.toList());
      return KuduPredicate.newInListPredicate(columnSchema, doubleValues);

    case DECIMAL:
      final List<BigDecimal> decimalValues = this.values.stream().map(o -> (BigDecimal) o).collect(Collectors.toList());
      return KuduPredicate.newInListPredicate(columnSchema, decimalValues);

    case BINARY:
      // @TODO: this is weird.
      final List<byte[]> binaryValues = this.values.stream().map(o -> (byte[]) o).collect(Collectors.toList());
      return KuduPredicate.newInListPredicate(columnSchema, binaryValues);
    default:
