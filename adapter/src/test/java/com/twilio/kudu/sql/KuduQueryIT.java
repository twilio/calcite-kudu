...
public class KuduQueryIT {
  ...
  public static Function1<Object, Object> MAP_RESPONSE_TWO_STRINGS = new Function1<Object, Object>() {
    ...
    public Object apply(Object r) {
      ...
      return new Object[] { row.getString(0), row.getString(1) };
    }
  };
  ...
  public void queryBaseTableForMessageSid() throws Exception {
    final CalciteKuduTable relTable = new CalciteKuduTableBuilder(KuduQueryIT.TABLE, testHarness.getAsyncClient())
        .build();
    ...
    final Enumerable<Object> results = relTable.executeQuery(
        ...
        false, new KuduScanStats(), new AtomicBoolean(false), MAP_RESPONSE_ONE_STRING, ALWAYS_TRUE, true);
    Iterator<Object> resultIter = results.iterator();

    Assert.assertTrue("Should have something to iterate over", resultIter.hasNext());
    Assert.assertEquals("Message sid should match", KuduQueryIT.FIRST_SID, resultIter.next());
    Assert.assertFalse("Should only have one row", resultIter.hasNext());
  }
  ...
  public void queryBaseTableForAccountSid() throws Exception {
    ...
    final Enumerable<Object> results = relTable.executeQuery(
        ...
        false, new KuduScanStats(), new AtomicBoolean(false), MAP_RESPONSE_TWO_STRINGS, ALWAYS_TRUE, false);
    Iterator<Object> resultIter = results.iterator();

    Assert.assertTrue("Should have something to iterate over", resultIter.hasNext());
    final Object[] firstRow = (Object[]) resultIter.next();
    Assert.assertEquals("Message sid should match first sid", KuduQueryIT.FIRST_SID, firstRow[0]);
    Assert.assertTrue("Should have a second row", resultIter.hasNext());
    final Object[] secondRow = (Object[]) resultIter.next();
    Assert.assertEquals("Message sid should match second sid", KuduQueryIT.SECOND_SID, secondRow[0]);

    Assert.assertEquals("First row should have two fields", 2, firstRow.length);
    Assert.assertEquals("Second row should have two fields", 2, secondRow.length);
    Assert.assertEquals("First row second field should match the account sid", KuduQueryIT.ACCOUNT_SID, firstRow[1]);
    Assert.assertEquals("Second row second field should match the account sid", KuduQueryIT.ACCOUNT_SID, secondRow[1]);
  }
  ...
  public void queryForInSids() throws Exception {
    ...
    // @TODO: we have the columnSchema in the setup, we don't need to grab the
    // table.
    // final KuduPredicate firstSid = KuduPredicate
    // .newComparisonPredicate(KuduQueryIT.TABLE.getSchema().getColumn(2),
    // KuduPredicate.ComparisonOp.EQUAL, KuduQueryIT.FIRST_SID);
    final CalciteKuduPredicate firstSid = new ComparisonPredicate(2, KuduPredicate.ComparisonOp.EQUAL,
    ...
    Assert.assertEquals(String.format("Should return two rows from the Kudu Table: %s", resultCollection), 2,
        resultCollection.size());
    Assert.assertTrue("First sid should be in the result set", resultCollection.contains(KuduQueryIT.FIRST_SID));
    Assert.assertTrue("Second sid should be in the result set", resultCollection.contains(KuduQueryIT.SECOND_SID));
  }
  ...
  public void testKuduScannerLimitPushdown() throws Exception {
    ...
    // since we are not sorting assert that the limit is not pushed down into the
    // kudu scanner
    ...
    // even though we are sorting we cannot push down the limit since there is an
    // offset
  ...
  public void kuduProjectTwoColumnsMapToSingleColumn() throws Exception {
    ...
    Assert.assertEquals("Message sid should match first sid", KuduQueryIT.FIRST_SID, firstRow);
    Assert.assertTrue("Should have a second row", resultIter.hasNext());
    final Object secondRow = resultIter.next();
    Assert.assertEquals("Message sid should match second sid", KuduQueryIT.SECOND_SID, secondRow);
  }
}
